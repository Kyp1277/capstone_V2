...
from modules.nlp import clean_text, extract_education_profile, extract_skills, extract_weighted_skills, infer_job_domains
...
def build_gap_reason(missing_skills, role_score, semantic_score, education_score=None):
    reasons = []

    if missing_skills:
        reasons.append(f"Skill yang belum terlihat di CV: {', '.join(missing_skills[:6])}.")
    ...
    if not reasons:
        reasons.append("Kecocokan sudah cukup baik, tetapi CV tetap perlu bukti pengalaman yang lebih spesifik.")
        ...
        processed_jobs.append(
            {
                ...
                "notFitReason": build_gap_reason(missing_job_skills, role_score if not is_auto_mode else semantic_score, semantic_score, education_score),
